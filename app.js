...
const APPWRITE_PROJECT_ID = "6aa13c840028943ee6b2";
const DATABASE_ID = "6aa13d0c00306354283c";
const TABLE_ID = "6aa13d3b00273118073b";     // the "expenses" table
...
// Turns an Appwrite row ($id, $createdAt, etc.) into the simple
...
async function getExpenses() {
  const response = await tablesDB.listRows({
    ...
    tableId: TABLE_ID,
    queries: [Appwrite.Query.orderDesc("date")],
  });
...
async function renderExpenses() {
  ...
  try {
    ...
    console.error("Couldn't load expenses from Appwrite:", err);
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    console.error("Couldn't save expense to Appwrite:", err);
